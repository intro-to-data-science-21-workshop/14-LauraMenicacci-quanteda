...
* try out some sentiment analysis for our subset
...
In the second plot, we converted frequencies to a new measure score ("sent_score") where parties with more negative words relative to positive ones get a sent_score<0 and parties with more positive words relative to negative ones get a sent_score>0. Parties with a sent_score=0 are the ones which do not have the word Brexit in their electoral programmes, thus the dictionary did not label them. 


```{r}
sent_df <- brexit_sent %>% 
  convert(to = "data.frame") %>% 
  subset(!(doc_id %in% c("51330", "51420", "51621"))) %>% 
  rename(party = doc_id) 

sent_df %>%
  pivot_longer(c(positive, negative)) %>% 
  ggplot(aes(x = value, y = party , fill= as.factor(name)))+
  geom_bar(position="dodge", stat="identity", width= 0.5) +
  #geom_segment(aes(xend=0, yend=party), color = "skyblue") +
  #geom_point(size=3, fill =  alpha = 0.5) +
...
sent_df %>% 
  pivot_longer(c(positive, negative)) %>% 
  ggplot(aes(x = reorder(party, sent_score), y = sent_score)) + 
